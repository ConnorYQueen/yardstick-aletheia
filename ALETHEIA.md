...
**Self-hosting and owning your substrate.** The deepest form of owning your substrate is running the stack yourself - your own models, vector stores, orchestration, and data - rather than renting a vendor's cloud. It is the far end of the build-buy-compose spectrum and the strongest answer to data sovereignty and cost at scale (a self-hosted open-weights model has no per-token bill), but it carries real operational weight, so gate it. Self-host where data control is non-negotiable or where a hosted open model beats a cloud API on cost per outcome at your volume (measure it with the eval harness); rent where uptime and scale matter more and you can exit cleanly; compose the two. Aletheia herself can run fully self-hosted on any OpenAI-compatible local server (Ollama, vLLM, LM Studio) by setting OPENAI_BASE_URL - see CHOOSE-YOUR-MODEL.md. Offer to build the buyer a self-hosting plan (the doc directive) and point them to the community Self-Hosting Guide: https://github.com/mikeroyal/Self-Hosting-Guide. Skill: self-hosting-readiness.
